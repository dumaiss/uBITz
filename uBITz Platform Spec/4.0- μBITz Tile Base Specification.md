...
This specification defines the **minimum contract** every Tile must meet, independent of its physical implementation or internal architecture (FPGA, MCU, ASIC, etc.).
...
- A Tile **MUST NOT**:
  - Hold READY indefinitely under normal operation or beyond the Host’s declared `ReadyMaxuS` budget.
  - Violate the Dock’s timing rules for set-up/hold of READY relative to other signals.

**Design guidance (non-normative): Prefer “no-wait” device behavior**

Tile designers SHOULD avoid stretching `/READY` for common register reads/writes because it stalls the bus and harms overall responsiveness.

Recommended patterns:

- Make normal register reads/writes complete immediately (no `/READY` stretch in the typical case).
- For slow operations (flash/SD access, decompression, long DSP, etc.), expose a **command/status** interface:
  - A write to `CMD` starts work and returns immediately.
  - `STATUS.BUSY` can be polled (or an IRQ raised on completion).
  - Results are read later from `DATA` registers/FIFOs.
- Avoid binding a μBITz bus transaction directly to slow peripheral bus latency (I²C, SPI erase, SD read); buffer internally and respond from local registers.
- If stretching `/READY` is used, document worst-case timing and ensure it fits within the Host’s declared `ReadyMaxuS`.
...
- Each Tile **MUST** conform to:
  - The voltage rails and current limits defined by the relevant Dock profile (Parallel).
- Tiles **MUST**:
...
This section is informative; actual electrical and timing limits are defined in each Dock profile (Parallel/Minimal). Tile designers **SHOULD** read the appropriate profile specification before finalizing a design.

## 3.1 Parallel profile (PCIe x16 slots)

- Tiles using the Parallel profile:
  - **MUST** follow the PCIe x16-based pin map and signal definitions in Dock Part 2.
  - **SHOULD** assume short, local backplanes with sub-GHz signaling and respect any skew/length recommendations.

## 3.2 Minimal profile (MCU bridge)
...
- It **implements at least one Dock profile** (Parallel/Minimal) as specified in the corresponding Dock profile document, and
