...
4. **Interrupts that fit retro mental models**  
   - Shared interrupt channels: `/INT_CH[3:0]` and `/NMI_CH[3:0]`.  
   - Optional **Mode-2-style acknowledge** using `/INT_ACK[3:0]`:  
...
6. **Stable, commodity connectors**  
   - **Serial:** M.2-style connectors and no weird mechanical straps/locks.  
...
7. **Single profile per backplane**  
   - A given Dock/backplane runs **one profile**: Serial *or* Parallel *or* Minimal.  
