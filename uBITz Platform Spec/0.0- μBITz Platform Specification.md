...
μBITz is designed for **retro-friendly, hobbyist-scale, non-safety-critical** systems.

μBITz targets environments where:
...
If your application requires any of the following, μBITz is **not** the right standard:
...
Author’s note, preserved for emphasis:

> “I’m going to personally hunt you down and smack you on the head if you use this standard for critical applications.” You have been warned. 😄

The **μBITz Dock** explicitly inherits this Platform design philosophy; individual subsystem specs may add extra constraints but not weaken these.

---

### 0.2.2 Why μBITz exists

μBITz was created to:
...
- **Improve portability**  
  The same driver and logical I/O model work across **Serial**, **Parallel**, and **Minimal** profiles.
...
### 0.2.3 Who this targets (and who it doesn’t)
...
- Firmware developers who want to **write drivers once** and reuse them on Serial, Parallel, and Minimal builds.
...
- PCI/PCIe compatibility (μBITz is **not** PCI/PCIe and does not aim to be).
...
1. **One logical model, many physiques**  
   All profiles (Serial, Parallel, Minimal) implement the **same logical I/O model**:  
...
3. **Scaled but bounded widths**  
   ...
   - **Serial profile:** backplane/devices **MUST** support the Host’s declared widths or fail enumeration.  
...
5. **Encoding that favors debuggability**  
   - Serial links use simple framed symbols (**4b + parity + 4b + parity**) rather than dense schemes like 8b/10b.  
   - The goal is “easy to probe and reason about” on hobbyist tools.

6. **Stable, commodity connectors**  
   - Parallel: PCIe x16 edge connector:  
	   - Do **not** reassign standard power/ground pins.  
	   - Use **3.3 V** and optional **5 V** only; **no 12 V** usage in μBITz designs.
	   
   - **Serial:** PCIe x16 edge connector:
	   - Exact same power and control signals as Parallel profile
	   - Adds the required signals for serial transmissions

6. **Multiple profile per backplane**  
   - A given Dock/backplane usually runs **one profile**: Serial *or* Parallel *or* Minimal.  
   ...
   - However, pinout designed such that Serial and Parallel MAY be supported on a single Dock
...
v1.0 of μBITz explicitly does **not** aim to support:

- Bus mastering, DMA, or cache coherency on the shared bus.
- Dynamic re-assignment of power/ground on standardized connectors.
- Mixed-profile backplanes (Serial + Parallel + Minimal in the same chassis at once).
...
These may be revisited in future versions or in separate, higher-complexity standards, but they are **out of scope** for μBITz v1.0.
...
- **Serial Profile**: PCIe x16 connector (signal reuse only) with LVDS framing (10-bit symbols)
...
- Interrupt channels: `INT_CH[1:0]`, `NMI_CH` (device→backplane)
- Interrupt acknowledge: `/INT_ACK` (backplane→device)
...
- Maskable interrupts: `CPU_INT[3:0]` ← routed from `INT_CH[1:0]` per slot
- Non-maskable interrupts: `CPU_NMI[1:0]` ← routed from `NMI_CH` per slot
- Acknowledge: `/CPU_ACK` → routed to `/INT_ACK` per slot
...
### 0.5.1 Memory Transaction Contract (CPU ↔︎ Bank)
...
### 0.5.2 I/O Transaction Contract (CPU ↔︎ Dock)
...
### 0.5.3 Interrupt Contract (Dock → CPU)
...
2. Execute enumeration sequence (see §0.4.1)
...
3. **μBITz Dock Specification v1.0** (provided)
    ...
    - Part 3: Serial profile (PCIe connector, LVDS, framed protocol)
...
**Bank (Memory Board)**: μBITz memory subsystem implementing platform word addressable memory

**Chip Select (CS)**: Signal indicating which device should respond to a bus operation

**CPU Board (Host)**: μBITz Host subsystem implementing platform personality
...
**Dock (Backplane)**: μBITz Dock subsystem implementing platform IO backplane

**Enumeration**: Discovery and configuration process at system startup

**Function**: Logical device type (video, sound, storage, etc.)

**Host (CPU Board)**: μBITz Host subsystem implementing platform personality
...
**Tile (Device Board)**: μBITz Tile implementing platform IO Devices
...
T2: R/W_ asserted → Data setup time
