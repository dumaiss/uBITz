# Part 3 — Parallel Profile (Parallel Bus)
...
* **Addressing:** The backplane selects a slot by `(IOWin, IOMask, OpSel)` on
  `A[7:0]` and forwards the **entire CPU I/O address** to the device unchanged.
...
## 3.2 Physical connector and power (summary)

This profile uses a **PCIe x16** edge connector mechanically, but reassigns
pins for a 32-bit I/O bus and per-slot interrupts. 

Here’s a concrete draft pin map you can drop into §3.2 for the Parallel profile.

Assumptions I baked in:

* “odd” = PCIe side **B** (component side), “even” = side **A** (solder side).
* All pins that are **Ground** or **3.3 V / 3.3 Vaux** in standard PCIe stay **GND/POWER**.
* Former **12 V** pins are **IO only** (no 12 V rail on the connector).
* Address bus lives mostly on the odd side; data bus mostly on the even side.
...
| 1    | even            | D->B            | NC    | NC           | CNot connected (must remain unconnected by host and device). |
...
| 7    | even            | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 8    | odd             | B->D            | POWER | VCC_3V3      | Main 3.3 V rail.                                             |
| 8    | even            | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
...
| 12   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 12   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 13   | odd             | B->D            | GND   | GND          | Ground.                                                      |
| 13   | even            | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 14   | odd             | B->D            | IO    | A[0]         | Address bit 0 (LSB).                                         |
| 14   | even            | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
...
| 74   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 74   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 75   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
...
| 78   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 78   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 79   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 79   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 80   | odd             | B->D            | GND   | GND          | Ground.                                                      |
| 80   | even            | D->B            | IO    | /INT_CH1     | Maskable interrupt channel 1 from device.                    |
| 81   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
| 81   | even            | D->B            | IO    | /NMI_CH      | Non-maskable interrupt channel from device.                  |
| 82   | odd             | B->D            | NC    | NC           | Not connected (must remain unconnected by host and device).  |
...
As a result, a Parallel-profile Dock or Tile designer can treat the signal table in §3.2 as a **direct physical mapping** of the logical interface described in Part 1:
...
## 3.3 Signal mapping to Core logical set
...
### 3.3.1 Bus ownership and tri-stating (normative)
...
## 3.4 Transaction semantics and timing
...
### 3.4.1 Example WRITE cycle (informative)
...
### 3.4.2 Example READ cycle (informative)
...
### 3.5.1 Per-slot lines and routing
...
### 3.5.2 Mode-2 acknowledge over Parallel
...
## 3.7 Compliance checklist — Device (Parallel)
...
## 3.8 Compliance checklist — Backplane (Parallel)
...
## 3.10 Reserved values & future-proofing
...
Here’s a draft Appendix C you can drop into the Parallel profile spec.

---

Nice, let’s wire this up properly.

Below is a **replacement Appendix C** for the Parallel Profile, with:

* Updated **C.1 / C.2** conformance matrices whose *How to verify* columns now explicitly assume:

  * Host self-test firmware,
  * A simple “echo / test” Tile,
  * A logic analyzer and scope.
* A new **C.3 Interop Test Checklist (informative)**, mirroring the Serial Profile’s checklist but adapted to the Parallel bus.

You can drop this at the end of the Parallel spec and tweak IDs/wording as needed.
...
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-1  | Powers all mandatory logic (enumeration + register bank) from **VCC_3V3**; may use **VCC_3V3_STBY** and optional **VCC_5V**, but must not *require* 5 V for basic operation. | 3.2, 3.7             | Power a slot with 3.3 V (and 3.3 V standby if used), leave 5 V off. Run Host self-test that enumerates the Tile and does basic register reads; confirm it works without 5 V.       |
| D-2  | Uses only the rails and pins defined in the Parallel pin map; all pins marked **NC** in §3.2 remain unbonded/undriven on the Tile.                                           | 3.2, 3.7             | Schematic/layout review: NC pads not routed. Continuity test NC pins on a bare board or assembled Tile; confirm open-circuit to all nets and supplies.                             |
| D-3  | Connects to `A[AddressBusWidth-1:0]` and ignores higher-order address pins; behavior is defined only up to declared `AddressBusWidth`.                                       | 3.3, 3.6, 3.7        | With test firmware, sweep addresses where only high bits differ (e.g. `0x0000` vs `0x8000` for 15-bit width). Use LA to confirm Tile only responds within the defined range.       |
| D-4  | Connects to `D[DataBusWidth-1:0]` and **never** drives higher-order data pins; high lanes must remain idle/high-Z from the device.                                           | 3.3, 3.3.1, 3.6, 3.7 | On a Host with 32-bit bus, perform READ cycles while monitoring all `D[31:0]` with an LA. Confirm only low `DataBusWidth` bits toggle; upper bits stay at pull level.              |
| D-5  | Obeys bus ownership: drives `D[]` only on READ cycles while `R/W_ = 1`, `/IORQ = 0` and its `/CS = 0`; never drives `D[]` on WRITE cycles.                                   | 3.3.1, 3.4, 3.7      | Connect Tile and a “bus sniffer” card or LA. Run Host test that randomly mixes reads and writes. Check that Tile’s output-enable on D matches only the legal READ windows.         |
| D-6  | Drives `/READY` only when selected (`/CS = 0`); must not pull `/READY` low when not addressed.                                                                               | 3.3.1, 3.4, 3.7      | Install two Tiles; have firmware repeatedly access only Tile A. With LA on `/CS`, `/READY`, confirm only A’s accesses cause wait-states. Tile B should never affect `/READY`.      |
| D-7  | Implements Core register map semantics under `WindowMap[]` and `OpSel`; responds only in its configured windows and ignores unmapped addresses.                              | 3.1, 3.4, 3.7        | After enumeration, have Host self-test program Tile windows and probe within/outside these ranges. Verify via LA that device only asserts `/READY` and drives `D[]` in-range.      |
| D-8  | Treats multi-byte (16/32-bit) I/O operations as **atomic**, with little-endian byte ordering on `D[]`.                                                                       | 3.4, 3.6, 3.7        | Use firmware to write 16/32-bit patterns (e.g. `0x1234`, `0x89ABCDEF`) and immediately read back. LA should show low byte on `D[7:0]`, contiguous bytes on higher lanes.           |
| D-9  | Exposes per-slot interrupt lines `INT_CH[1:0]` and `NMI_CH` with electrical behavior per Core (open-drain/level as required by platform).                                    | 3.2, 3.5, 3.7        | With scope/LA and known pull-ups on Dock, toggle device interrupt bits in firmware. Verify correct idle level, active level, and that lines can be wire-OR’d safely.               |
| D-10 | If vectored interrupts (`IntAckMode = 0x01`) are supported, provides an 8-bit **Vector Register** and returns the vector on `D[7:0]` while `/INT_ACK` is asserted.           | 3.5.2, 3.7           | Configure CPU descriptor for Mode-2. Trigger device INT, then have Host issue an ack cycle. Capture `/INT_ACK`, `A[]`, `D[7:0]` with LA; confirm expected vector byte is observed. |
| D-11 | When `IntAckMode = 0x00` (polling mode), ignores `/INT_ACK` and does not change any internal state in response to it.                                                        | 3.5.3, 3.7           | Configure Host for polling mode and still pulse `/CPU_ACK`. Verify via device status registers and LA that `/INT_CHx` behavior is unchanged by the ack pulse.                      |
| D-12 | Participates in enumeration via `SLOT_SCL`/`SLOT_SDA` and exposes a valid Device Descriptor (Function IDs, widths, interrupts, etc.).                                        | 3.2, 3.6, 3.7        | Use a simple I²C dump tool on the Host side. Read the Tile’s descriptor and check fields match the hardware (bus widths, IRQ channels, function type).                             |
| D-13 | Meets platform timing for setup/hold relative to `/IORQ`, `R/W_`, `A[]`, `D[]`, and `/READY` at the target bus frequency.                                                    | 3.4, 3.9, 3.7        | With scope/LA at max rated bus clock, run stress test (random read/write patterns). Measure setup/hold on signals at the slot; confirm margins meet Host timing diagrams.          |
...
| ID   | Requirement (summary)                                                                                                                                                           | Spec §          | How to verify                                                                                                                                                                             |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B-1  | Provides **VCC_3V3** to every slot; may provide **VCC_3V3_STBY** and optional **VCC_5V**. Respects documented per-slot and total current limits.                                | 3.2, 3.8, 3.9   | Connect programmable loads or Tiles with known consumption. Measure slot voltages and currents under worst-case population; confirm they remain within spec and no rail droop or reset.   |
| B-2  | Implements the Parallel pin map exactly; all pins marked **NC** in §3.2 are unconnected.                                                                                        | 3.2, 3.8        | Schematic/layout review of the backplane connector. Use continuity tests from connector fingers to the PCB; NC pads should not connect to any net, plane, or test point.                  |
| B-3  | Exposes `A[31:0]`, `D[31:0]`, `/IORQ`, `R/W_`, `/READY`, `/RESET`, `/CS[slot]`, `INT_CH[1:0]`, `NMI_CH`, `/INT_ACK`, `/CPU_INT[3:0]`, `/CPU_NMI[1:0]`, `/CPU_ACK` as specified. | 3.2, 3.3, 3.8   | With no Tiles or with a passive “breakout” Tile, probe slot pins while Host exercises basic cycles. Confirm presence, direction, and idle levels for all required signals.                |
| B-4  | Decodes `(IOWin, IOMask, OpSel)` from `A[7:0]` and asserts **exactly one** `/CS[slot]` (or none) for each mapped I/O cycle.                                                     | 3.1, 3.4, 3.8   | Plug in multiple Tiles or LED test cards. Run firmware that sweeps addresses. With LA, check that at most one `/CS[x]` is low for any `/IORQ` and that unmapped ranges assert none.       |
| B-5  | Forwards the **full CPU address** and `R/W_` to the selected slot unchanged.                                                                                                    | 3.1, 3.3, 3.4   | Use a Tile that can latch and expose the last address seen (e.g., via debug register). Compare CPU address vs. Tile’s latched value for a variety of accesses; they should match exactly. |
| B-6  | Aggregates `/READY` so only the **selected** slot can stretch the CPU cycle; non-selected Tiles must not affect CPU-visible `/READY`.                                           | 3.3.1, 3.4, 3.8 | Install two Tiles; have Tile A hold `/READY` low for a programmable time on access. Use LA to confirm CPU `/READY` tracks only Tile A accesses and is unaffected by Tile B behavior.      |
| B-7  | For unmapped windows, does not assert any `/CS`; CPU reads return all-ones (`0xFF`, `0xFFFF`, or `0xFFFFFFFF` depending on data width).                                         | 3.4, 3.6, 3.8   | With at least one Tile present, perform reads to clearly unmapped addresses. Confirm via LA that no `/CS` toggles and Host firmware sees all-ones patterns.                               |
| B-8  | Preserves atomicity of 16/32-bit I/O operations; must not reorder byte lanes or split wide accesses into visible partial writes to the Tile.                                    | 3.4, 3.6, 3.8   | Attach a Tile that exposes internal per-byte write strobes. Using firmware, do 16/32-bit writes while watching strobes with LA; confirm all bytes commit together, never partially.       |
| B-9  | Routes per-slot `INT_CH[1:0]` and `NMI_CH` to `/CPU_INT[3:0]` and `/CPU_NMI[1:0]` according to `IntRouting[]` from the CPU descriptor.                                          | 3.1, 3.5, 3.8   | Program routing tables; for each channel on each slot, assert interrupts in firmware and monitor CPU interrupt pins with LA. Confirm mapping matches routing configuration.               |
| B-10 | Serializes interrupts at the CPU: only one source is actively presented at a time; additional requests stay pending at Tiles until the active one is cleared.                   | 3.5.1, 3.8      | Make two Tiles assert different interrupt channels simultaneously. Observe CPU interrupts and device status: CPU should service them one-by-one while both remain asserted at the slots.  |
| B-11 | In Mode-2 (`IntAckMode = 0x01`), tracks the active source (slot + channel) and routes `/CPU_ACK` to that slot via `/INT_ACK` only.                                              | 3.5.2, 3.8      | With LA on `/CPU_ACK`, `/INT_ACK[x]` lines, trigger an interrupt, then ack. Verify only the slot that asserted the interrupt sees `/INT_ACK` during the ack cycle.                        |
| B-12 | In Mode-2, issues a vector read and forwards the 8-bit vector byte from the Tile on `D[7:0]` during the ack cycle.                                                              | 3.5.2, 3.8      | Use a Tile with known vector. On ack, capture `A[]`, `/CS`, `D[7:0]` with LA. Confirm the backplane holds `/CS` and the CPU sees the Tile’s vector byte at the correct cycle.             |
| B-13 | If multiple claimants or no claimant exist for a given ack, returns `0xFF` as vector and does not assert `/INT_ACK` to any single slot.                                         | 3.5.2, 3.8      | Configure two Tiles to assert the same channel. Trigger ack. LA should show CPU reading `0xFF` on `D[7:0]` while no `/INT_ACK[x]` line is pulsed to any specific slot.                    |
| B-14 | When `IntAckMode = 0x00` (polling mode), ignores `/CPU_ACK` and never asserts `/INT_ACK` in response.                                                                           | 3.5.3, 3.8      | Set CPU descriptor to polling mode and deliberately toggle `/CPU_ACK`. Use LA to confirm `/INT_ACK` lines remain idle for all slots.                                                      |
| B-15 | Maintains signal integrity suitable for the target bus frequency (grounding, return paths, controlled impedance and fanout).                                                    | 3.2, 3.9, 3.10  | SI review (field solver or vendor guidelines), then run at or above target MHz with several worst-case Tiles installed; check for error-free operation and absence of visible ringing.    |
...
If you’d like, we can next turn this into a little **“Compliance Test Plan”** document (separate from the spec) with step-by-step instructions you could literally hand to someone in a lab: which firmware to flash, which patterns to run, and where to put the logic analyzer probes.
