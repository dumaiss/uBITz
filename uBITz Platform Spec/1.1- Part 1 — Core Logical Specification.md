...
| `/DMA_REQ`               | D→B         | Bus mastering request by device (per-slot in physical profiles). Forwarded by Dock/backplane to the Host.                                                                                                                                                                                              |
| `/DMA_GNT`               | C→B         | Host “bus-safe” indication for bus mastering (active-low). Asserted only when the Host guarantees CPU/Host bus drivers are Hi-Z; Dock/backplane may use it as an interlock for ownership/direction switching. The Dock MAY broadcast this signal to all slots as an input; devices MUST NOT drive it. |
...
#### 1.3.1 Bus Mastering Hooks (Reserved) (Normative)

- `/DMA_REQ` is **per-slot** (device→backplane). Devices **MUST NOT** assert it in v1 (leave NC / deasserted).
- `/DMA_GNT` is **single-driver** (Host→backplane). Only the Host may drive it.
- `/DMA_GNT=0` means: Host guarantees CPU/Host drivers are Hi-Z and it is safe for the backplane to switch bus direction/ownership (if implemented).
- `/DMA_GNT=1` means: not granted / not safe (default).
- v1 behavior:
  - Backplane/Dock ignores `/DMA_REQ`.
  - Host holds `/DMA_GNT` deasserted (logic 1) continuously.
...
### 1.6.0 /READY bounded wait budget and timeout completion

The backplane MAY stretch a Core I/O cycle by holding CPU-visible `/READY=0`, but `/READY` **MUST NOT** be held longer than the Host-declared maximum wait budget:

- The Host CPU Descriptor **MUST** declare `ReadyMaxuS` (maximum time the backplane may hold CPU-visible `/READY=0` for a single Core I/O cycle qualified by `/IORQ=0`).
- The backplane **MUST** release CPU-visible `/READY→1` within `ReadyMaxuS`.

**Timeout completion (normative):**  
If the selected device has not completed the transaction before `ReadyMaxuS` expires, the backplane **MUST** complete the bus cycle deterministically:

- Release CPU-visible `/READY→1` so the CPU can finish the cycle.
- For **reads**: the backplane MUST source **all-ones** on the active data width (`0xFF`, `0xFFFF`, or `0xFFFFFFFF`) for that cycle.
- For **writes**: the write MUST be treated as **not committed** (i.e., failed).
- The backplane **MUST** latch a timeout fault condition (reporting mechanism is platform-defined).

### 1.6.1 Write cycle (R/W_=0)

1. **Start:** CPU drives `A[…]`, `D[…]`, `R/W_=0`, then asserts `/IORQ=0`.
2. **Claim (dance):** Backplane immediately drives CPU-visible `/READY=0` to claim the cycle, then asserts exactly one `/CS[n]=0` to the selected device once decode is stable.
...
2. **Claim (dance):** Backplane immediately drives CPU-visible `/READY=0` to claim the cycle, then asserts `/CS[n]=0` once decode is stable.
...
- Backplane **MUST** release CPU-visible `/READY→1` within `ReadyMaxuS` (or apply timeout completion per §1.6.0).
...
- **Parallel/Minimal (Parts 2–3):** The physical bus **always exposes** `A[AddressBusWidth-1:0]` and `D[31:0]`; backplane/devices MUST adapt to the CPU’s declared widths.
...
struct CPUDescriptor {
    ...
    uint8_t  Reserved1[6];       // All set to 0x00
    ...
    uint32_t ReadyMaxuS;          // Max time Dock may hold /READY low for any                                     // I/O cycle (µs) Dock MUST complete the cycle 
                                  // (normal or timeout-complete) within this                                      // bound.
...
- Required guarantees:
    ...
    - The backplane/device path MUST latch the effective address (`A[AddressBusWidth-1:0]`) and operation (`R/W_`) at cycle start.
    - /CS assertion timing:
...
- Declare `ReadyMaxuS`, handle `/READY` stretching within that budget, and tolerate platform-enforced timeout completion semantics.
...
### **1.19.1 Function-Based Addressing**  (Informative)

**Concept:**

- The CPU issues I/O cycles using an address `A[AddressBusWidth-1:0]`.
- The CPU descriptor defines a set of WindowMap entries `(IOWin, Mask, OpSel)`.
- The backplane selects a single device slot by applying the Core rule in §1.4:
  `(A & Mask) == (IOWin & Mask)` and `OpSel` matches `R/W_`.

Example:
- CPU software uses I/O addresses, for example 0x30-0x3F for "sound chip"
- Backplane translates Function + Instance → Physical Slot
- Same software works regardless of which slot card is in

**Important note:**
- There is no fixed requirement that “window bits” must be `A[7:4]` or that the system uses 16 windows.
- Any subset of bits (within `AddressBusWidth`) may be used by the platform’s WindowMap policy via `Mask`.


**Address Decode:**

```
CPU I/O Address: A[7:0] <-- example: a Z80
  A WindowMap entry for SOUND could be:
  { Function=SOUND, Instance=0, IOWin=0x00000030, Mask=0x000000F0, OpSel=WRITE }

Worked Example (AddressBusWidth=8):
  CPU executes: OUT (0x35), A
  Backplane computes:
    (0x35 & 0xF0) == (0x30 & 0xF0) → true, select SOUND instance 0
  Backplane action:
    Assert /CS[2] (/CS for slot 2 for function SOUND)
    Forward : WRITE, ADDR=0x35, DATA=A
...
- **Parallel/Minimal:** same rule—exactly one FAULT LED on the backplane; no error channel or POST code is required or defined by the Core. If no EEPROM descriptor is used in the Minimal profile, FAULT LED is optional.
