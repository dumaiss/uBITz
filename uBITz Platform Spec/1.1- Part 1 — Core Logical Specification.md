...
2. **Claim:** Backplane immediately drives CPU-visible `/READY=0` to claim the cycle, then asserts exactly one `/CS[n]=0` to the selected device once decode is stable.
...
2. **Claim:** Backplane immediately drives CPU-visible `/READY=0` to claim the cycle, then asserts `/CS[n]=0` once decode is stable.
