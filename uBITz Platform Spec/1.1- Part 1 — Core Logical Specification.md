...
- Interrupt model (`/INT_CH[3:0]`, `/NMI_CH[3:0]`, `/INT_ACK[3:0]`) including Mode-2-style acknowledge
...
- **Enumeration bus:** The profile-defined control bus used to read descriptors (I²C recommended).
...
| `R/W_` | C→B
B→D | Read/Write select: **1 = READ**, **0 = WRITE** (active-low W). C→B is always parallel; B→D is profile-specific. |
| `A[AddressBusWidth-1:0]` | C→B
B→D | Address bus from CPU; backplane forwards to device (profile-specific encoding on B→D). |
| `D[DataBusWidth-1:0]` | C⇄B
B⇄D | Data bus, little-endian byte lanes. Always parallel for C→B but profile specific for B→D |
| `/READY` | D→B
B→C | Wait-state stretch (active low). |
...
| `/RESET` | B→C
B→D | Synchronous reset for devices on the backplane. |
| `/INT_CH[3:0]` | D→B | ***Per-slot*** Maskable interrupt request lines. From a device to the backplane, before routing. |
| `/CPU_INT[3:0]` | B→C | Maskable interrupt request lines from backplane to CPU after routing. |
| `/NMI_CH[3:0]` | D→B | ***Per-slot*** Non-maskable interrupt request lines. From device to backplane, before routing. |
| `/CPU_NMI[3:0]` | B→C | Non-maskable interrupt request lines from backplane to CPU after routing. |
| `/CPU_ACK[3:0]` | C→B | Active-low interrupt acknowledge for the corresponding INT channel from CPU to backplane, before routing. |
| `/INT_ACK[3:0]` | B→D | ***Per-slot*** Active-low interrupt acknowledge for the corresponding INT channel from backplane to device after routing. |
| **Enumeration bus** | — | Control channel to read descriptors (location & pins are profile-specific). |
| TOTAL | 24-104 | Minimum: 
* 24 signals: 8 bits data, 8 bits address, no interrupt, 8 IO Signals
Maximum: 
* 104 signals: 32 bits data, 32 bits address, 32 interrupts, including ack, 8 IO Signals |
...
During a Mode-2 acknowledge, `/CS` **MUST NOT** be de-asserted; the backplane routes `/CPU_ACK[k]` →  `/INT_ACK[k]` to **the slot currently asserting `INT_CH[k]`**

**Per-slot interrupt architecture:**

- Each slot exposes its own set of up to 8 interrupt lines (4 INT + 4 NMI)
- Physical implementation: 32 traces total (4 slots × 8 channels)
...
- Backplanes MUST implement between 0 and 8 interrupt channels per slot
- Valid configurations: 0 (no interrupts), 1-8 channels
...
- The backplane selects a Window by the following operation:
    - Let the IO Port be A[7:0], the MASK be IOMask as specified in the CPU Descriptor and OP be the operation to perform Read = 1, Write = 0, ANY = FF; then
    `(A[7:0] & MASK) == (IOWin & IOMask)` and OP permits the operation
...
2. CPU asserts `/CPU_ACK[k]=0` **while also asserting `/IORQ=0` and `R/W_=1`**.
3. Backplane routes `/CPU_ACK[k]` to the **same slot** currently asserting `INT_CH[k]`, keeps `/CS[n]=0`, pulls `/READY=0`, and sources a **read** from that slot.
4. The device (or backplane on its behalf) drives the **8-bit vector index** on `D[7:0]`.
5. Backplane releases `/READY→1`; CPU samples the vector; CPU then deasserts `/IORQ→1` and `/CPU_ACK[k]→1`; backplane deasserts `/CS[n]→1`.
...
### 1.8.1 Lines and levels

- `INT_CH[3:0]` and `NMI_CH[3:0]` are interrupt request lines.
- `CPU_INT[3:0]` and `CPU_NMI[3:0]` are asserted by the backplane, following routing rules.
...
- **`IntAckMode = 0x01` (Mode-2-style):**
    - The CPU acknowledges a specific channel `k` by asserting **`/CPU_ACK[k]=0`** for an acknowledge cycle, which asserts the `INT_ACK[3:0]` on the device side that initiated the interrupt. This is determined by keeping track of the currently asserted`INT_CH[3:0]` and `/CS`
...
### 1.8.3 Priority & concurrency

- If multiple `INT_CH[x]` are asserted, priority is **implementation-defined** in the CPU/firmware.

### 1.8.4 Per-Slot Channel Architecture

Each slot provides **UP TO** 8 independent interrupt channels:

- `Slot_N.INT_CH[3:0]`: UP TO 4 maskable interrupt channels
- `Slot_N.NMI_CH[3:0]`: UP TO 4 non-maskable interrupt channels
...
- Total backplane traces: UP TO 32 (4 slots × UP TO 8 channels)

**Examples:**

- Full implementation: 32 traces (all channels on all slots)
- Minimal implementation: 4 traces (Slot 0: INT_CH[0], NMI_CH[0]; Slot 1: same)
...
- During Mode-2 ack, backplane routes `/CPU_ACK[k]` to the specific slot that asserted `INT_CH[k]`
...
### 1.8.5 Interrupt Routing Lookup

When a device in Slot N asserts channel Ch:

1. Backplane identifies (Function, Instance) assigned to Slot N
2. Determine channel identifier:
    - If Ch is INT_CH[k]: channel_id = k (0x00-0x03)
    - If Ch is NMI_CH[k]: channel_id = 0x10 + k (0x10-0x13)
3. Search IntRouting[] for entry matching (Function, Instance, channel_id)
4. If found: Assert DestPin per Mode and Stretch_us
5. If not found: Ignore (no CPU interrupt generated)

**Example:**

```
Device in Slot 2: Function=VIDEO, Instance=0
Device asserts: Slot2.NMI_CH[1]

Lookup: (Function=VIDEO, Instance=0, Channel=0x11)
IntRouting[3] = 
	{ Function=VIDEO, Instance=0, Channel=0x11, DestPin=CPU_NMI[0], Mode=EDGE }
Result: Assert CPU_NMI[0] with edge pulse
```

---

## 1.9 Bus width disclosure (CPU → backplane)
...
## 1.10 Window & register behavior (device side)
...
## 1.11 Enumeration model & descriptors
...
struct CPUDescriptor {
    ...
    // I/O Window Mapping (128 bytes, 16 windows × 8 bytes each)
    struct {
        uint8_t  Function;        // Function ID (0x00=none, 0x01-0xFF)
        uint8_t  Instance;        // Instance number (0-6)
        uint8_t  IOWin;           // IO Window Address for specified function see Sec.1.4
        uint8_t  Mask;            // Mask for IO Windows address see Sec.1.4
    ...
    // Example:  IO Address 0xBF & Mask 0xE0 = C0 --> match IO Win to C0
    //           if Operation is read (0) or write (1) will select different devices

    // Interrupt Routing (64 bytes, 8 channels × 8 bytes each)
    struct {
	      uint8_t  Function;        // Function ID (0x00=none, 0x01-0xFF)
 	      uint8_t  Instance;        // Instance ID (0-6)
        uint8_t  Channel;         // Interrupt channel capability (bitmask)
                                  // bit[3:0]  => INT_CH0..INT_CH3 (LSB is INT_CH0)
																	// bit[7:4]  => NMI_CH0..NMI_CH3 (bit4 is NMI_CH0)
															 	// Set a bit to 1 if the device MAY assert that channel.
														  		// 0x00 = no interrupt capability.
														  		// Examples:
																	// 0b0000'0101  => INT_CH0 and INT_CH2
																	// 0b0001'0000  => NMI_CH0 only
																	// 0b1000'0011  => NMI_CH3 + INT_CH0 + INT_CH1
        uint8_t  DestPin;                                         
        uint8_t  Mode;            // 0x00=EDGE, 0x01=LEVEL
        uint8_t  Stretch_us;      // Stretch length for pulse mode
        uint8_t  Reserved[2];     // All set to 0x00
    } IntRouting[8];              // [0-3]=INT_CH[0-3], [4-7]=NMI_CH[0-3]
};
...
- Lookup is by (Function, Instance) key, not array index
- Order of entries in IntRouting[] is not significant
- If multiple entries have the same (Function, Instance), behavior is undefined
(implementations MAY use first match or fail enumeration)
- Unmatched (Function, Instance) pairs result in no interrupt routing
(device interrupts are ignored unless validation is enabled)
...
struct PeripheralDescriptor {
    ...
    struct {
        ...
        uint8_t  IntChannel;          // Interrupt channel capability (bitmask)
                                      // bit[3:0]  => INT_CH0..INT_CH3 (LSB is INT_CH0)
														    			// bit[7:4]  => NMI_CH0..NMI_CH3 (bit4 is NMI_CH0)
															 	 // Set a bit to 1 if the device MAY assert that channel.
														  	     	// 0x00 = no interrupt capability.
...
### 1.11.3 Enumeration flow (normative)
...
- For each enumerated device:
    ...
    b. Determine channel index from bitmask bit position:
        - Bits [3:0] → Channel = 0x00-0x03 (INT_CH[0-3])
        - Bits [7:4] → Channel = 0x10-0x13 (NMI_CH[0-3])
...
## 1.14 Compliance checklist (Core)
...
- For `IntAckMode=Mode2`, assert exactly one `/CPU_ACK[k]` during an ack cycle and consume an **8-bit** vector index from `D[7:0]`.
- Assert `/IORQ=0` to qualify every Core I/O cycle, hold it until `/READY=1`, then release.
- For Mode-2, assert `/IORQ=0`, `R/W_=1`, and `/CPU_ACK[k]=0` for the entire acknowledge cycle.
...
- During Mode-2 ack, assert /INT_ACK[k] to the specific slot that asserted INT_CH[k]
...
## 1.15 Reserved values & future-proofing
...
## 1.16 Informative examples

### 1.16.1 32-bit write to a 16-bit timer pair
...
### 1.16.2 Mode-2-style interrupt

- Device asserts `/INT_CH[1]` when a frame is ready.
- CPU acks by asserting `/CPU_ACK[1]=0`.
...
### **1.16.3 Coleco-style shared ranges (write→SOUND, read→HID):**
...
## 1.17 Minimal annex: recommended defaults (informative)
...
## **1.18. ADDRESSING & ROUTING**

### **1.18.1 Function-Based Addressing**
...
## **1.19. INTERRUPT ROUTING**

**Lines:** `INT_CH[3:0]` and `NMI_CH[3:0]` are dedicate per device. `/CPU_ACK[3:0]` acknowledges **INT** channels (not NMIs).

**Who goes on which line? (Channel Assignment Table, CAT):**

- At enumeration time, the backplane creates a **Channel Assignment Table (CAT)** that maps **each device function** (i.e., each claimed `WindowID`) to **one of the 4 INT channels**.
- NMI sources, if any, follow the same policy into `NMI_CH[3:0]` (platform-defined; often reserved for highest-priority events).

**Mode-2-style acknowledge :**

- CPU selects the asserted channel **k** and asserts **`/INT_ACK[k]=0`**. Selected channel is the same as the interrupt channel.
...
- **Ack**: CPU → `/CPU_ACK[x]` for the chosen channel; device/backplane returns **vector index** on `D[7:0]`.

### 1.19.1 Signal Topology
...
```
Slot N:
  INT_CH[3:0]  : up to 4 maskable interrupt channels
  NMI_CH[3:0]  : up to 4 non-maskable interrupt channels

Total backplane traces: up to 32 (4 slots × 8 channels)
...
```
CPU card exposes pins (via HiRose connector):
  CPU_INT[3:0]  : CPU interrupt input pins
  CPU_NMI[3:0]  : CPU NMI input pins
  CPU_ACK[3:0]  : CPU interrupt acknowledge outputs (Mode-2 only)
...
### **1.19.2 Routing Configuration**

**CPU Card EEPROM Policy:**

```
For each board channel (INT_CH[3:0], NMI_CH[3:0], INT_ACK[3:0]):
  DestPin:   Which CPU_INT[0-3] or CPU_NMI[0-3] to drive
  Mode:      EDGE or LEVEL
...
Example (8085 CPU):
  ...
  INT_CH2 → CPU_INT[1], EDGE, 2µs      (RST7.5, needs pulse stretch)
  INT_CH3 → CPU_INT[0], LEVEL, 0µs     (INTR, via 8259A PIC)
  NMI_CH0 → CPU_NMI[0], EDGE, 0µs      (TRAP)
  NMI_CH2 → CPU_NMI[0], EDGE, 10µs     (Power-fail, longer pulse)

Example (68000 CPU):
  INT_CH0 → CPU_INT[0], LEVEL          (IPL0, autovector level 1)
  INT_CH1 → CPU_INT[1], LEVEL          (IPL1, autovector level 2)
  INT_CH2 → CPU_INT[2], LEVEL          (IPL2, autovector level 3)
...
Device declares which channels it uses:
  IntChannels:  Bitmask [3:0] (which INT_CH lines)
  NmiChannels:  Bitmask [3:0] (which NMI_CH lines)

Example (Sound card):
  IntChannels = 0b0001 (uses INT_CH0 for buffer-ready events)
  NmiChannels = 0b0000 (does not use NMI)

```

### **1.19.3 Backplane Interrupt Logic**
...
### **1.19.4 Interrupt Service Routine (ISR) Flow**
...
### 1.19.5 Enumeration outcome & fault indication
