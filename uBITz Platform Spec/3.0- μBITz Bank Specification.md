...
- It sees at most **four right-aligned logical spaces**:
  - **RAM space** (selected by `/MEM0_CS = 0 + /MEM1_CS = 1`)
  - **ROM space 0** (selected by `/MEM0_CS = 1 + /MEM1_CS = 0`)
  - **ROM space 1** (selected by `/MEM0_CS = 0 + /MEM1_CS = 0`)
  - **ROM space 2** (selected by `/MEM0_CS = 1 + /MEM1_CS = 1`)
- It sees the CPU/canonical address on `A[]` and may decode only low address bits internally as an offset (mirroring permitted).
...
- **μBITz Host Specification v1.0**  
  (address space mapper inputs; Dock decoding of `/MEM0_CS` and `/MEM1_CS`)
...
- Accepts a canonical address on `A[]`, uses Dock-provided space selects, and may decode only low address bits internally as an offset (mirroring permitted) to return/store data.
...
- Require knowledge of platform-specific maps (C64, Apple II, etc.); it decodes only what it needs from canonical `A[]`.
- Decode I/O addresses or respond to `/IORQ`.
- Implement platform-visible “bank switching” on its own:
  - Banking is the responsibility of the μBITz Host.
...
**Inputs (from Dock/Host bus):**

- `A[OffboardAddrWidth-1:0]`  
  CPU/canonical address for the cycle (zero-extended as needed). `OffboardAddrWidth` is declared in the Bank Descriptor.
...
- `/CS` – Active-low slot select from Dock
...
`/MEM0_CS` and `/MEM1_CS` are driven by the Dock when the Bank slot is selected.

### 2.2 Requirements

A v1.0 Bank **MUST**:

1. Treat incoming addresses as canonical; it may decode only low address bits internally as offsets (mirroring/aliasing permitted).
2. Respond only when **all** are true:
   - `/CS = 0` (slot selected by the Dock)
   - `/MEM0_CS` and `/MEM1_CS` indicate a valid Bank space (RAM/ROM0/ROM1/ROM2)
   - `R/W_` indicates direction, and `A[]`/`D[]` are used accordingly.
   The Dock guarantees that `/CS` is only asserted for valid Bank memory cycles; therefore the Bank **MUST** ignore `/MREQ` and `/IORQ`.
1. Maintain `D[]` tri-stated:
   - During reset.
   - Whenever no valid Bank cycle is in progress.
   - On cycles targeted at other devices/spaces.
   - When `/CS = 1`, the Bank **MUST NOT** drive `D[]` and **MUST NOT** assert `/READY` (leave inactive/Hi-Z as applicable).
...
### 2.3 Offset Addressing and Mirroring

When selected (`/CS = 0` and `/MREQ = 0`), the Bank **SHALL** interpret `A[]` using its decoded low address bits as an offset within the selected space (RAM/ROMx).

Bank implementations **MAY** decode only a subset of low address bits (K). In that case, `bank_offset = A[K-1:0]`.

If fewer bits are decoded than presented, mirroring/aliasing and unused capacity are permitted and considered valid behavior.

Note: Physical Bank capacity may exceed CPU addressability; unreachable capacity is acceptable unless paging/bank-switch is implemented.
...
- `RAMAddrWidth` (bits)  
  Number of address lines implemented for RAM (e.g., 18 ⇒ 256 KiB locations).
- `ROMAddrWidth` (bits)  
  Number of address lines implemented for ROM (e.g., 18 ⇒ 256 KiB locations).
- `DataBusWidth` (bits)  
  8 / 16 / 32.  
...
- Host asserts `/MREQ`, places `A[]` and `D[]` (for writes); the Dock drives `/MEM0_CS` and `/MEM1_CS_` when the Bank slot is selected.
- Bank:
...
3. The Bank **MUST NOT**:
   - Hold `/READY` low indefinitely under normal operation; CPU-visible stretches **MUST** fit within the Host’s declared wait budget (`ReadyMaxuS`) or trigger platform timeout completion semantics.
...
- Still present the Host with a **static** `/READY`-based interface:
  - The Host does not need to know when refresh is happening, only that `/READY` will be deasserted when the operation is complete.
...
- A single contiguous **RAM space** of `2^RAMAddrWidth` bytes, starting at offset 0, when `/MEM0_CS = 0 and /MEM1_CS = 1`.
- Three contiguous **ROM spaces** of `2^ROMAddrWidth` bytes, starting at offset 0, when either of:
	- `/MEM0_CS = 1 and /MEM1_CS = 0`
	- `/MEM0_CS = 0 and /MEM1_CS = 0`
	- `/MEM0_CS = 1 and /MEM1_CS = 1`

**Requirement:**  
The Bank **MUST** behave as if these spaces are linear, 0-based arrays of words/bytes internally, even if it derives offsets by decoding only low address bits.

### 5.2 RAM/ROM Address Width and Capacity

The `RAMAddrWidth` and `ROMAddrWidth` fields constrain how many unique locations are directly addressable from the Host:

- Maximum directly addressed locations per space:
  - `MaxLocations = 2^ROMAddrWidth or 2^RAMAddrWidth` 
- For example:
  - `ROMAddrWidth = 18` ⇒ 256 KiB locations (in ROM).
  - `RAMAddrWidth = 20` ⇒ 1024 KiB locations (in RAM).
  - With `DataBusWidth = 8`, that’s up to 1024 KiB RAM and 256 KiB ROM visible at once.
...
- Completely transparent to the Bank:
  - The Bank only ever decodes the low address bits it needs within its declared capacity (mirroring permitted).
...
- Simple glue logic for `/MEM0_CS`, `/MEM1_CS`, `/MREQ`, `R/W_`, `/READY`.
- `RAMAddrWidth` typically 16–20 bits.
- `ROMAddrWidth` typically 16-18 bits.
...
- Map RAM and Flash into separate parts of the same `/MEM0_CS` or `/MEM1_CS` space internally, as long as the external view is contiguous.
...
2. It implements the **logical interface** of §2:
   - Canonical addresses on `A[]` with Dock-provided selects; internal offset decode may use only low bits.
   - `/MREQ`, `/MEM0_CS`, `/MEM1_CS`, `R/W_`, `D[]`, `/READY` with the described behavior.
1. It provides contiguous RAM/ROM spaces as described in §5:
