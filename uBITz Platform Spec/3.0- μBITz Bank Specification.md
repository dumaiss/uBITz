...
- It sees at most **four right-aligned logical spaces**:
  ...
  - **ROM space 0** (selected by `/MEM0_CS = 1 + MEM0_CS = 0`)
  - **ROM space 1** (selected by `/MEM0_CS = 1 + MEM0_CS = 0`)
  - **ROM space 2** (selected by `/MEM0_CS = 1 + MEM0_CS = 0`)
...
- **μBITz Host Specification v1.0**  
  (address space mapper, /MEM0_CS and /MEM1_CS_ generation)
...
2. Respond only when:
   - `/MREQ` is asserted *and* a valid memory address space `/MEM0_CS` and `/MEM1_CS` is asserted.
...
**Core Connector Overlay (Host ↔ Bank)**

The Core (Host/Bank) connectors use the same PCIe x16 mechanical pin numbering and base mapping as the Parallel Profile Tile slots (see Part 2 – Parallel Profile).

All pins not listed in the table below retain their Parallel Profile names, directions, and electrical requirements.  

The following pins are **reassigned** on Core connectors:
...
| 75   | odd  | SER_B2D_N             | `/MEM0_CS`          | Host → Bank     | Bit 1 of memory address space on the Bank. |
| 78   | odd  | SER_D2B_P             | `/MEM1_CS`          | Host → Bank     | Bit 2 of memory address space on the Bank. |
...
- Host connectors do **not** implement the Serial FRLVDS link.    
- `NC` pins MUST be left unconnected on Host and Bank in v1.x.    
- All other pins (`A[31:0]`, `D[31:0]`, `R/W_`, `/READY`, `/RESET`, power, GND, `GPIO_DBG[0..5]`, etc.) are identical to the Parallel Profile Tile slots.

**Rationale**:
Having both the Bank and the Host share the exact same pin out allows for swapping the two cards on a backplane. Given they are both PCIe x16 connectors, being able to swap the connectors eliminates possible damage from mating the cards in the wrong slots.
...
- Host asserts `/MREQ` + `/MEM0_CS` and/or `/MEM1_CS_`, places `A[]` and `D[]` (for writes).
- Bank:
...
- Three contiguous **ROM spaces** of `2^ROMAddrWidth` bytes, starting at offset 0, when either of:
	- `/MEM0_CS = 1 and /MEM1 = 0`
	- `/MEM0_CS = 0 and /MEM1 = 0`
	- `/MEM0_CS = 1 and /MEM1 = 1`
