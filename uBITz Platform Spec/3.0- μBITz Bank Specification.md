...
- It sees at most **two right-aligned logical spaces**:
  - **RAM space** (selected by `/RAM_CS`)
  - **ROM space** (selected by `/ROM_CS`)
...
- **μBITz Host Specification v1.0**  
  (address space mapper, /RAM_CS and /ROM_CS_ generation)
...
- `/RAM_CS` – Active-low RAM space select  
- `/ROM_CS` – Active-low ROM space select  
  (Exactly one of `/RAM_CS_` or `/ROM_CS_` may be active for a valid memory cycle; both may be inactive when no Bank access is happening.)
...
2. Respond only when:
   - `/MREQ` is asserted *and* exactly one of `/RAM_CS` or `/ROM_CS` is asserted.
...
| 75   | odd  | SER_B2D_N             | `/RAMSEL`           | Host → Bank     | Selects RAM region on the Bank.        |
| 78   | odd  | SER_D2B_P             | `/ROMSEL`           | Host → Bank     | Selects ROM / boot region on the Bank. |
...
- Host asserts `/MREQ` + `/RAM_CS` or `/ROM_CS_`, places `A[]` and `D[]` (for writes).
- Bank:
...
- A single contiguous **RAM space** of `2^RAMAddrWidth` bytes, starting at offset 0, when `/RAM_CS = 0`.
- A single contiguous **ROM space** of `2^ROMAddrWidth` bytes, starting at offset 0, when `/ROM_CS = 0`.
...
- Simple glue logic for `/RAM_CS`, `/ROM_CS`, `/MREQ`, `R/W_`, `/READY`.
...
- Map RAM and Flash into separate parts of the same `/RAM_CS` or `/ROM_CS_` space internally, as long as the external view is contiguous.
...
2. It implements the **logical interface** of §2:
   ...
   - `/MREQ`, `/RAM_CS`, `/ROM_CS_`, `R/W_`, `D[]`, `/READY` with the described behavior.
