...
**Note – Core (Host/Bank) connectors:**  

The μBITz link between Host and Bank reuses the same PCIe x16 mechanical connector and base pin assignments as the Parallel Tile slots.  

On the Bank and Host connectors, the pins labelled `SER_B2D_*`, `SER_D2B_*`, and `SER_REFCLK_*` are **not** used for Serial and are instead repurposed for the Host–Bank memory control interface (see: _Core Connector Overlay_ in the Host/Bank specification).
...
- **CLK_DOCK[0:3]** (B→D) – Four independent reference clocks sourced from the Dock, distributed to all Tiles, Host, and Bank.
    - CLK_DOCK0: Pin 8 (even side)
    - CLK_DOCK1: Pin 12 (odd side)
    - CLK_DOCK2: Pin 13 (even side)
    - CLK_DOCK3: Pin 14 (even side)
