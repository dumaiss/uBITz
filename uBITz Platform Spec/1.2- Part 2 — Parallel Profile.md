...
| 1    | even            | D→B             | SENSE | /DRV5V    | Strap low only if the card may **drive 5V outputs** onto any shared bus/control line. Dock pull-up in AON.             |
...
| 10   | odd             | B->D            | POWER | VCC_3V3   | Main 3.3 V rail.                                                                                                       |
...
| 18   | odd             | D→B             | IO    | /DMA_REQ  | Bus-master request from device (active-low). (Reserved/ignored in v1 unless enabled later.) LEAVE NC UNTIL IMPLEMENTED |
| 18   | even            | B->D            | GND   | GND       | Ground.                                                                                                                |
| 19   | odd             | B->D            | IO    | A[3]      | Address bit 3.                                                                                                         |
| 19   | even            | B↔D             | IO    | D[2]      | Data bit 2.                                                                                                            |
| 20   | odd             | B->D            | IO    | A[4]      | Address bit 4.                                                                                                         |
| 20   | even            | B→D             | IO    | /DMA_GNT  | Bus-master grant to device (active-low). _(Reserved/ignored in v1 unless enabled later.)_. LEAVE NC UNTIL IMPLEMENTED  |
...
| 74   | even            | D->B            | IO    | /CPU_ACK  | Host → backplane acknowledge for vectored interrupt/IntAck-mode style cycles (active-low).                             |
| 75   | odd             | B->D            | IO    | /MEM0_CS  | Bit 1 of memory region select.                                                                                         |
| 75   | even            | B->D            | IO    | /CPU_INT0 | Backplane → Host interrupt input 0.                                                                                    |
...
| 78   | odd             | B->D            | IO    | /MEM1_CS  | Bit 2 of memory region select.                                                                                         |
| 78   | even            | B->D            | IO    | /CPU_INT1 | Backplane → Host interrupt input 1.                                                                                    |
| 79   | odd             | D->B            | IO    | /CPU_INT2 | Backplane → Host interrupt input 2 (active-low).                                                                       |
| 79   | even            | B->D            | IO    | /CPU_NMI0 | Backplane → Host NMI input 0                                                                                           |
| 80   | odd             | B->D            | GND   | GND       | Ground.                                                                                                                |
| 80   | even            | D->B            | IO    | /INT_CH1  | Maskable interrupt channel 1 from device.                                                                              |
| 81   | odd             | B->D            | IO    | /CPU_INT3 | CPU Interrupt Line 3                                                                                                   |
| 81   | even            | D->B            | IO    | /NMI_CH   | Non-maskable interrupt channel from device.                                                                            |
| 82   | odd             | B->D            | IO    | /CPU_NMI1 | Backplane → Host NMI input 1 (active-low).                                                                             |
| 82   | even            | D->B            | SENSE | /SENS3V   | Strap low only if the card is **3.3V-only / not 5V-tolerant**. Dock pull-up in AON.                                    |
...
- Pin 1 (even side): /DRV5V
- Pin 82 (even side): /SENS3V
