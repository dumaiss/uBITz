...
| 72   | B          | B→D             | IO    | /DMA_GNT  | Bus-master grant to device (active-low). _(Reserved/ignored in v1 unless enabled later.)_. LEAVE NC UNTIL IMPLEMENTED  |
| 72   | A          | B↔D             | IO    | D0        | Data bit 0 (LSB).                                                                                                      |
| 73   | B          | B->D            | IO    | /RESET    | Global system reset from backplane (active-low).                                                                       |
...
| 75   | A          | B->D            | IO    | /INT_ACK  | Interrupt acknowledge from backplane to device (active-low).                                                           |
| 76   | B          | B->D            | IO    | /MEM1_CS  | Bit 2 of memory region select.                                                                                         |
| 76   | A          | B->D            | IO    | /CPU_INT0 | Backplane → Host interrupt input 0.                                                                                    |
| 77   | B          | B->D            | IO    | R/W_      | Read/Write direction (1=READ, 0=WRITE).                                                                                |
| 77   | A          | B->D            | IO    | /CPU_INT1 | Backplane → Host interrupt input 1.                                                                                    |
| 78   | B          | B->D            | IO    | /IORQ     | I/O cycle qualifier from backplane (active-low).                                                                       |
| 78   | A          | D->B            | IO    | /CPU_INT2 | Backplane → Host interrupt input 2 (active-low).                                                                       |
| 79   | B          | D->B            | IO    | /READY    | Device wait-state indication to backplane (active-low).                                                                |
| 79   | A          | B->D            | IO    | /CPU_INT3 | CPU Interrupt Line 3                                                                                                   |
| 80   | B          | B->D            | IO    | /CS       | Per-slot chip select from backplane (active-low).                                                                      |
| 80   | A          | D->B            | IO    | /NMI_CH   | Non-maskable interrupt channel from device.                                                                            |
| 81   | B          | D->B            | IO    | /INT_CH0  | Maskable interrupt channel 0 from device.                                                                              |
| 81   | A          | B->D            | IO    | /CPU_NMI0 | Backplane → Host NMI input 0                                                                                           |
| 82   | B          | D->B            | IO    | /INT_CH1  | Maskable interrupt channel 1 from device.                                                                              |
