...
| 1    | even            | -               | NC    | RESERVED     | Reserved for future use. Must be left unconnected.           |
...
| 7    | even            | B->D            | IO    | CLK_HOST     | Reference clock from Host (passed through Dock to Tiles).    |
| 8    | odd             | B->D            | POWER | VCC_3V3      | Main 3.3 V rail.                                             |
| 8    | even            | B->D            | IO    | CLK_DOCK0    | Reference clock 0 from Dock.                                 |
...
| 12   | odd             | B->D            | IO    | CLK_DOCK1    | Reference clock 1 from Dock.                                 |
| 12   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 13   | odd             | B->D            | GND   | GND          | Ground.                                                      |
| 13   | even            | B->D            | IO    | CLK_DOCK2    | Reference clock 2 from Dock.                                 |
| 14   | odd             | B->D            | IO    | A[0]         | Address bit 0 (LSB).                                         |
| 14   | even            | B->D            | IO    | CLK_DOCK3    | Reference clock 3 from Dock.                                 |
...
| 74   | odd             | B->D            | IO    | /MREQ        | Memory request qualifier (active-low).                       |
| 74   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 75   | odd             | B->D            | IO    | /MEM0_CS     | Bit 1 of memory region select.                               |
...
| 78   | odd             | D->B            | NC    | NC           | Reserved for future.                                         |
| 78   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 79   | odd             | D->B            | IO    | /MEM1_CS     | Bit 2 of memory region select.                               |
| 79   | even            | B->D            | GND   | GND          | Ground.                                                      |
| 80   | odd             | B->D            | GND   | GND          | Ground.                                                      |
| 80   | even            | D->B            | IO    | /INT_CH1     | Maskable interrupt channel 1 from device.                    |
| 81   | odd             | B->D            | NC    | NC           | Reserved for future.                                         |
| 81   | even            | D->B            | IO    | /NMI_CH      | Non-maskable interrupt channel from device.                  |
| 82   | odd             | B->D            | NC    | NC           | Reserved for future.                                         |
| 82   | even            | B->D            | GND   | GND          | Ground.                                                      |

| PIN# | Side | Parallel Profile Name | Core Host/Bank Name | Dir (Host↔Bank) | Notes                                  |
| ---- | ---- | --------------------- | ------------------- | --------------- | -------------------------------------- |
| 74   | odd  | SER_B2D_P             | `/MREQ`             | Host → Bank     | Memory request qualifier (active-low). |
| 75   | odd  | SER_B2D_N             | `/MEM0_CS`          | Host → Bank     | Bit 1 of memory region select.         |
| 78   | odd  | SER_D2B_P             | `/MEM1_CS`          | Host → Bank     | Bit 2 of memory region select.         |
| 79   | odd  | SER_D2B_N             | NC                  | —               | Reserved for future Core/Bank control. |
| 81   | odd  | SER_REFCLK_P          | NC                  | —               | Reserved for future Core/Bank status.  |
| 82   | odd  | SER_REFCLK_N          | NC                  | —               | Reserved for future Core/Bank status.  |
=======
>>>>>>> d22314ca29cf38a372a71552124eabd2dc01561e

| PIN# | Side (A/B) | Direction (B/D) | Type  | Name          | Description                                                                                                                                                                                                                                                |
| ---- | ---------- | --------------- | ----- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | B          | B→D             | POWER | +5V_MAIN      | Main 5 V rail (slot power).                                                                                                                                                                                                                                |
| 1    | A          | D→B             | SENSE | /VIO_5V       | Strap low only if the card may **drive 5V outputs** onto any shared bus/control line. Dock pull-up in AON.                                                                                                                                                 |
...
| 11   | A          | D→B             | SENSE | /SLOT_PRSNT   | Device present sense pin. Always tie to GND. Dock pull-up in AON.                                                                                                                                                                                          |
...
| 71   | B          | D→B             | IO    | /DMA_REQ      | Per-slot bus-master request from device (active-low). Reserved/ignored in v1. **Devices MUST leave NC / deasserted until bus mastering is enabled.**                                                                                                       |
| 71   | A          | B→D             | GND   | GND           | Ground.                                                                                                                                                                                                                                                    |
| 72   | B          | B→D             | GND   | GND<br>       | Ground                                                                                                                                                                                                                                                     |
| 72   | A          | B↔D             | IO    | D0            | Data bit 0 (LSB).                                                                                                                                                                                                                                          |
| 73   | B          | B⇄D             | IO    | /DMA_GNT<br>  | Host “bus-safe” indication (active-low). **Driven only by the Host slot** when CPU/Host drivers are Hi-Z and the bus is safe to switch (future bus mastering). Dock/backplane broadcasts to all slots as input.                                            |
| 73   | A          | D→B             | IO    | /READY<br>    | Device wait-state indication to backplane (active-low).<br>                                                                                                                                                                                                |
| 74   | B          | D→B             | IO    | /CPU_ACK      | Host → backplane acknowledge for vectored interrupt/IntAck-mode style cycles (active-low).<br><br>**Host IntAck strobe** (active-low). Asserted by Host during interrupt-acknowledge / vector-fetch cycles; Dock uses it to initiate device ack selection. |
| 74   | A          | B→D             | IO    | /RESET        | Global system reset from backplane (active-low).<br>                                                                                                                                                                                                       |
...
| 76   | A          | B→D             | IO    | /MEM0_CS      | Bit 0 of memory region select.                                                                                                                                                                                                                             |
...
| 78   | A          | B→D             | IO    | /CS           | Per-slot chip select from backplane (active-low).                                                                                                                                                                                                          |
| 79   | B          | B→D             | IO    | /CPU_NMI0     | Backplane → Host NMI input 0                                                                                                                                                                                                                               |
| 79   | A          | B→D             | IO    | /INT_ACK      | Interrupt acknowledge from backplane to device (active-low).<br><br>/CPU_ACK marks an interrupt-acknowledge cycle (Host→Dock). /INT_ACK selects/authorizes the responding device (Dock→Device).                                                            |
| 80   | B          | B→D             | IO    | /CPU_NMI1     | Backplane → Host NMI input 1.                                                                                                                                                                                                                              |
| 80   | A          | D→B             | IO    | /NMI_CH       | Non-maskable interrupt channel from device.                                                                                                                                                                                                                |
| 81   | B          | B→D             | IO    | /MREQ         | Memory request qualifier (active-low).                                                                                                                                                                                                                     |
| 81   | A          | D→B             | IO    | /INT_CH0      | Maskable interrupt channel 0 from device.                                                                                                                                                                                                                  |
| 82   | B          | B→D             | IO    | /IORQ         | I/O cycle qualifier from backplane (active-low).                                                                                                                                                                                                           |
| 82   | A          | D→B             | IO    | /INT_CH1      | Maskable interrupt channel 1 from device.                                                                                                                                                                                                                  |

#### 2.2.0 Bus Mastering Signals (Reserved) (Normative)

- `/DMA_REQ` is per-slot; the backplane forwards it to the Host (future arbitration).
- `/DMA_GNT` is a single Host-driven signal; the backplane uses it as an interlock for any future bus direction/ownership switching.
- Only the Host may drive `/DMA_GNT`. All other slots **MUST** treat `/DMA_GNT` as input-only.
- v1: Devices **MUST NOT** assert `/DMA_REQ`. Host **MUST** hold `/DMA_GNT` deasserted.
...
Docks MAY keep `+3V3_MAIN` and `+5V_MAIN` disabled until the Dock power-up sequence completes. Devices MUST assume slot rails are absent while `/RESET` is asserted and until the Dock explicitly enables main rails.
...
---
### 2.2.1.1 Slot Presence and Per-Slot VIO Selection (Normative)

The Parallel Profile supports both 3.3 V and 5 V *signaling* at the slot interface while keeping the Dock’s internal logic in a protected 3.3 V domain. This is achieved using two strap-style pins per slot:

- `/SLOT_PRSNT#` — slot presence (enables/disables the slot electrically)
- `/VIO_5V#` — slot interface logic level (selects the slot-facing I/O voltage `VIO_SLOT[n]`)

These pins are configuration straps only. They MUST NOT be used to source power to devices.

#### Dock/backplane requirements

- The Dock/backplane MUST provide a weak pull-up (typ. 10 kΩ) from each strap pin to a Dock-local always-on 3.3 V domain (e.g., `+3V3_AON`).
- The Dock/backplane MUST be able to read these strap pins even when slot power rails are disabled.
- The Dock/backplane MUST implement per-slot buffering/level translation between its internal 3.3 V domain and the slot-facing domain `VIO_SLOT[n]`.

#### Device/card requirements

- `/SLOT_PRSNT#` MUST be tied to the device ground plane (asserted low) whenever a card is installed. This is the responsibility of the device.
- `/VIO_5V#` MUST be strapped as follows:
  - FLOAT (pulled up by Dock) → 3.3 V signaling for this slot (default)
  - GND (asserted low) → 5 V signaling for this slot

Straps MUST be implemented passively (GND or FLOAT). Devices MUST NOT require slot power rails to produce valid strap levels.

#### Required Dock behavior

Presence:

- If `/SLOT_PRSNT#` is deasserted (logic 1): the slot is empty.
  - The Dock/backplane MUST keep that slot electrically isolated by default:
    - All slot buffers/transceivers for that slot MUST be disabled (/OE inactive).
  - `VIO_SLOT[n]` may default to 3.3 V; it is ignored while the slot is disabled.

VIO selection:

- If `/SLOT_PRSNT#` is asserted (logic 0): a card is present.
- If `/VIO_5V#` is asserted (logic 0): the Dock MUST set `VIO_SLOT[n] = 5 V`.
- If `/VIO_5V#` is deasserted (logic 1): the Dock MUST set `VIO_SLOT[n] = 3.3 V`.

Notes (informative):

- `/VIO_5V#` declares the slot *signaling* level only. It does not imply anything about power consumption or which power rail(s) the card uses internally.
- A “3.3 V signaling” card is expected to be safe at 3.3 V I/O levels; a “5 V signaling” card may drive/require 5 V I/O levels at the μBITz interface. The Dock provides level translation to its internal 3.3 V logic domain in both cases.

#### Buffered Dock requirement (profile-level)

A Parallel-profile Dock/backplane MUST maintain an internal **3.3 V logic domain** and MUST level-translate signals between that internal domain and each slot’s `VIO_SLOT[n]`.

At minimum, this applies to all μBITz digital interface signals that cross between Dock logic and a slot, including (but not limited to) `A[]`, `D[]`, `/CS`, `R/W_`, `/IORQ`, `/RESET`, `/READY`, interrupt lines, and clock outputs, as implemented by the backplane.

#### Power-up sequencing interaction (normative)

- Slot power rails (e.g., `+3V3_MAIN`, `+5V_MAIN`) MAY be disabled by the Dock until the Dock is ready to power the system.
    
- Strap pins MUST remain readable in that state via Dock-provided pull-ups to the always-on 3.3 V domain.
    
- Backplane buffer/level-shifter components MUST be selected and wired such that **unpowered** slots and/or unpowered slot-facing rails do not back-power the Dock’s internal 3.3 V domain (e.g., devices with partial-power-down “Ioff” behavior, or equivalent protection).
...
- Clock outputs presented _to a slot_ are **VIO_SLOT[n] CMOS** … (3.3 V or 5 V depending on the slot strap).
...
* Backplanes MUST:
  * Connect only the lower `AddressBusWidth` and `DataBusWidth` lanes to the
    CPU board.
  * Route all 32 `A[]` / `D[]` lanes on the connector between slots as defined
    by the profile.
  * Treat unused lanes as reserved; they MUST NOT carry unrelated signals and
    MUST NOT be repurposed as sideband flags.
...
- Allows a single physical connector/pinout to support multiple Host families
  while preserving a simple electrical backplane.

### 2.6.1 Default State of Unused Address/Data Lanes (Normative)

The Parallel Profile connector exposes 32 address lanes (`A[31:0]`) and 32 data lanes (`D[31:0]`), while a given Host may declare narrower logical widths via its descriptor (`AddressBusWidth`, `DataBusWidth`).

To ensure deterministic behavior when some lanes are not driven (e.g., narrower Hosts, devices that tri-state, or unpopulated lanes), the Dock/backplane MUST enforce a defined default state on undriven lanes:

- Any `A[i]` lane that is not driven by the current bus owner MUST resolve to logic `0` at the backplane’s internal bus fabric.
- Any `D[i]` lane that is not driven by the current data source for the transaction MUST resolve to logic `0` at the backplane’s internal bus fabric.

Implications:

- Hosts and devices MAY leave unused high-order lanes unconnected (NC).
- Reads from narrower devices (e.g., 8-bit or 16-bit data sources) MUST return `0` on unused upper data bits when observed on the 32-bit backplane bus.

Implementation note (informative):

- This requirement is typically satisfied by weak pull-down biasing (e.g., 47 kΩ–100 kΩ) on the Dock/backplane side of the buffered `A[]` and `D[]` nets, combined with transceivers that tri-state cleanly when not enabled.
