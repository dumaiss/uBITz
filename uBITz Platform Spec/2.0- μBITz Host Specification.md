...
4. Implement an **Address Space Mapper** that converts CPU addresses into:
   ...
   - Corresponding control signals (`/MREQ`, `/IORQ`, `/MEM0_CS`, `/MEM1_CS`, etc.)
...
1. For any **memory** cycle issued toward the Bank:
   - One of memory address space selection `/MEM0_CS`, `/MEM1_CS` **MUST** be asserted low.
...
| ---- | ---- | --------------------- | ------------------- | --------------- | -------------------------------------- |
| 11   | odd  | /INT_ACK              | /CPU_ACK            | Host → Dock     | Interrupt Acknowlege                   |
...
| 77   | even | /INT_CH0              | /CPU_INT0           | Dock → Host     | CPU Interrupt Line 0                   |
| 78   | odd  | SER_D2B_P             | `/MEM1_CS`          | Host → Bank     | Bit 2 of memory region select.         |
| 79   | odd  | SER_D2B_N             | /CPU_INT2           | Dock → Host     | CPU Interrupt Line 2                   |
| 80   | even | /INT_CH1              | CPU_INT1            | Dock → Host     | CPU Interrupt Line 1                   |
| 81   | odd  | SER_REFCLK_P          | /CPU_INT3           | Dock → Host     | CPU Interrupt Line 3                   |
| 81   | even | /NMI_CH               | /CPU_NMI0           | Dock → Host     | CPU NMI Line 0                         |
| 82   | odd  | SER_REFCLK_N          | /CPU_NMI1           | Dock → Host     | CPU NMI Line 1                         |
...
Outputs (to μBITz side):
  ...
  /MREQ, /IORQ
  /MEM0_CS, /MEM1_CS                -- or equivalent encoded space ID
...
3. Asserting the proper combination of `/MREQ`, `/IORQ`, `/MEM0_CS`, `/MEM1_CS` lines.    
...
3. It generates `/MREQ`, `/IORQ`, `/MEM0_CS`, `/MEM1_CS_`, `R/W_`, `A[]`, and `D[]` according to this specification.    
