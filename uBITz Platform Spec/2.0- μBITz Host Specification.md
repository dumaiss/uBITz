...
4. Implement an **Address Space Mapper** that converts CPU addresses into:
   ...
   - Corresponding control signals (`/MREQ`, `/IORQ`, `/RAM_CS`, `/ROM_CS`, etc.)
...
- `A[AddressBusWidth-1:0]`  
  0-based **offset** within the selected memory space (RAM or ROM or IO).
...
1. For any **memory** cycle issued toward the Bank:
   - Exactly one of `/RAM_CS`, `/ROM_CS` **MUST** be asserted low.
...
Outputs (to μBITz side):
  ...
  /MREQ, /IORQ
  /RAM_CS, /ROM_CS                -- or equivalent encoded space ID
...
3. Asserting the proper combination of `/MREQ`, `/IORQ`, `/RAM_CS`, `/ROM_CS` lines.    
...
3. It generates `/MREQ`, `/IORQ`, `/RAM_CS`, `/ROM_CS_`, `R/W_`, `A[]`, and `D[]` according to this specification.    
...
- Space selects:    
    - RAM: `/RAM_CS = 0`, `/ROM_CS = 1`, `/IORQ = 1`, `/MREQ = 0`        
    - ROM: `/ROM_CS = 0`, `/RAM_CS = 1`, `/IORQ = 1`, `/MREQ = 0`        
    - IO: `/IORQ = 0`, `/MREQ = 1`, `/RAM_CS = 1`, `/ROM_CS = 1`
