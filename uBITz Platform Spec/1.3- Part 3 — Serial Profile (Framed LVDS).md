...
The Serial Profile assumes the PCIe x16 connector and pin map defined in Part 2 (§2.2) as the **mechanical ground truth**. This Part does **not** move any signals to new pins; it only:

- assigns specific PCIe contacts to LVDS lanes (`SER_B2D_*`, `SER_D2B_*`, `SER_REFCLK_*`) as listed in Part 2, and    
- defines how μBITz I/O transactions are framed and encoded over those lanes.
    
All other pins — including `SLOT_SCL/SLOT_SDA`, `/IORQ`, address/data lines, reset, interrupts, and `GPIO0–GPIO5` — are wired identically to the Parallel Profile and follow the electrical rules in Part 2.

### 3.2.1 Connector mapping vs Parallel Profile

The Serial Profile does **not** introduce a new connector. It reuses the
PCIe x16 pin map defined in Part 2 (§2.2) and assigns the following pins
to the framed LVDS link:
...
All other pins (power rails, `SLOT_SCL/SLOT_SDA`, `/RESET`, `/CS`, `A[]`,
`D[]`, `/INT_CH[1:0]`, `/NMI_CH`, `/INT_ACK`, etc.) are wired exactly as
in the Parallel Profile (§2.2). The Serial Profile only changes **how
I/O transactions are transported** between Dock and Tile; it does not
change Slot numbering, enumeration, interrupt topology, or the physical
location of those signals on the connector.

A Dock that supports both profiles MUST:

- Implement the `SER_*` pins per this Part for Serial-profile Tiles.
- Leave `SER_*` electrically idle for Tiles that do **not** claim the
  Serial Profile at enumeration.
- Preserve the behavior of all non-`SER_*` pins exactly as defined by
  the Parallel pin map.

A Tile that implements the Serial Profile MUST connect its LVDS PHY only
to the `SER_*` pins and MUST treat all other connector pins according
to Part 2 and Part 1 (Core).

### 3.2.2 Sideband GPIO reuse (GPIO0–GPIO5)

The Serial Profile reuses the optional sideband pins `GPIO0–GPIO5` defined in Part 2 (§2.2.3) without changing their electrical or behavioral rules:

* They remain 3.3 V open-drain, multi-drop signals.
* Serial-profile Docks and Tiles **MUST** satisfy all normative requirements of §2.2.3.
* Production Serial Tiles **MUST** function correctly with all of `GPIO0–GPIO5` left unconnected.

The framed LVDS transport defined in this Part does not encode or interpret these lines in any way; they are simply parallel sideband signals. 

In the reference Dock implementation, `GPIO0–GPIO5` are routed to the Dock management MCU and used to provide a simple “homebrew monitor” (enumeration inspection, manual resets, firmware upload hooks, etc.), but these behaviors are **informative only** and not required for Serial Profile conformance.
