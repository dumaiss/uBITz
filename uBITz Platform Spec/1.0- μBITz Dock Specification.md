...
- **Backplane compliance (Parallel/Minimal)**: exposes full `A[AddressBusWidth-1:0]`, `D[DataBusWidth-1:0]`; for v1.x, `AddressBusWidth` is 8/16/32.; provides `/RD`, `/WR`, `/READY`, `/INT_CH[3:0]`, `/NMI_CH[3:0]`, `/INT_ACK[3:0]`, I²C, `/RESET`. Minimal includes the **parallel↔serial bridge** to the MCU.
