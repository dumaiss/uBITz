# Part 2 — Serial Profile (Framed LVDS)
...
- **Error classification:** A parity-invalid 10-bit value matching a **reserved control token** (2.3.2) is a **control**; any other parity-invalid value is a **symbol error**.

### 2.3.2 Control tokens (reserved, parity-invalid)
...
### 2.3.3 DC balance and run-length
...
### 2.3.4 Rationale: Parity-10 vs. 8b/10b and CRC (informative)
...
## 2.4 Frame format and semantics
...
Each octet on wire is still encoded as Parity-10 (4+P,4+P) per §2.3.
...
### 2.4.1 Request header (B→D) — Frame on wire (with parity shown)
...
### 2.4.1.1 CONTROL octet mapping (quick reference)
...
### 2.4.1.2 Worked frames **with parity bits shown**
...
### 2.4.1.3 Decoder cheat-sheet (nibble parity)
...
## 2.5 Transaction mapping to Core
...
- **Widths:** Data width is the Core transaction width (`SZ`). The device **MUST** treat multi-byte ops as atomic per 2.4 and Core rules.
...
- **Per-slot interrupt wiring:** Each slot exposes dedicated physical interrupt lines device→backplane: `INT_CH[3:0]` and `NMI_CH[3:0]`. The LVDS link carries no interrupt signaling; it is used only for framed requests/responses. The backplane routes these lines to the CPU per **IntRouting**.
- **Mode-2 ack over Serial (vector read at conventional endpoint):**
    1. On a CPU `/CPU_ACK[k]`, the backplane identifies the **asserting slot** from the per-slot lines and **holds `/CS`** to that slot. It also **forwards the ack** to that slot (slot-local **`/INT_ACK[k]`** asserted for the duration of the cycle).
    2. The backplane issues a **READ** transaction to the device’ **vector read endpoint**:
        ...
        - **FLAGS:** `0x00`
            ...
            > This endpoint is only interpreted as a vector read while the slot-local **`/INT_ACK[k]`** is asserted; otherwise, a read at `ADDR=0x00` is treated as a normal device register per the device’s map.
            > 
...
- **NMI:** `NMI_CH[3:0]` are not vectored. The backplane **MUST NOT** perform a Vector Endpoint read for NMI; devices **MUST** ignore `/INT_ACK[k]` during an NMI event.

---

## 2.8 Compliance checklist — Device (Serial)

A conformant Serial-profile **device** MUST:

- Power its LVDS SERDES/PHY from **slot 3V3 VCC**; no dedicated LVDS rail.
- Implement **Parity-10** (2.3), **SOF/EOF/IDLE/ERR** tokens (2.3.2), the header fields (2.4.1), **commit-on-EOF**, and the one-frame read reply (2.4.2).
- Provide a valid **Device Descriptor** and function claims for windows; support declared bus widths or fail enumeration.
- Drive INT_CH[3:0]/NMI_CH[3:0] per Core electrical rules and return the vector index in response to a **vector read at `ADDR=0x00` while `/INT_ACK[k]` is asserted** if Mode-2 is claimed.

---

## 2.9 Compliance checklist — Backplane (Serial)
...
- Provide pull-ups/conditioning for per-slot INT_CH[3:0]/NMI_CH[3:0].

---

## 2.10 Timing (informative)
...
## 2.11 Reserved values & future-proofing
...
*(Helpers `emit_token_bits`, `emit_octet_bits`, `emit_bytes_bits` just wrap `encode_octet()` and serialize halves/10-bit tokens MS-to-LS or LS-first per §2.3.)*
...
- **Level assert:** Device asserts `INT_CH[k]` → CPU sees interrupt; during `/CPU_ACK[k]`, backplane holds `/CS` and asserts slot-local `/INT_ACK[k]`.
- **Vector read (Mode-2):** While `/INT_ACK[k]` asserted, backplane issues vector read (`CONTROL=0x01`, `ADDR=0x00`, `FLAGS=0x00`); device returns vector (0xFF if unclaimed).
- **Multiple claimants:** Two slots assert same `INT_CH[k]` → backplane returns `0xFF` without querying any slot.
- **NMI:** `NMI_CH[x]` not vectored; no vector read performed; devices ignore `/INT_ACK[k]`.
...
| --- | --- | --- | --- |
| D-1 | SERDES/PHY powered from slot **3V3** | 2.2 | Inspect BOM/power tree; measure 3V3 draw at slot. |
| D-2 | Implements **Parity-10** (nibble even parity) | 2.3, A.3/A.4 | Transmit/receive vectors: 0x00, 0xFF, 0xA5; check halves and parity. |
| D-3 | Recognizes control tokens **SOF/EOF/IDLE/ERR** (bit-true) | 2.3.2, A.1 | Send each token; verify state transitions and no data emission. |
| D-4 | Little-endian **on wire** for ADDR/DATA | 2.4 | Probe a 16/32-bit op and confirm byte order. |
| D-5 | **Commit on EOF** for writes; pre-EOF writes do **not** take effect | 2.4, 2.4.3 | Mid-frame ERR → no state change; full frame → state updated. |
| D-6 | **One** outstanding transaction per slot; no unsolicited frames | 2.4 | Attempt to pipeline; device must not respond until prior completes. |
| D-7 | HEADER layout: CONTROL/ADDR/FLAGS with FLAGS high-nibble 0 | 2.4.1 | Send non-zero FLAGS high nibble; device must ignore. |
| D-8 | Read response framing: **SOF | DATA | EOF** (no header) |
| D-9 | Handles in-frame error/abort correctly (no more octets after **ERR**) | 2.4.3 | Inject ERR; verify immediate stop and stable FSM. |
| D-10 | **INT_CH/NMI_CH** electrical behavior per Core | 2.7, 2.8 | Level/edge tests; open-drain/pull-ups confirmed at slot. |
| D-11 | Mode-2: while **/INT_ACK[k]** asserted, READ at `ADDR=0x00` returns 8-bit vector (0xFF if not claimant) | 2.7 | Drive interrupt; ack; verify vector byte; test non-claim = 0xFF. |
| D-12 | Ignores vector read when **/INT_ACK** deasserted (acts as normal reg) | 2.7 | Read `0x00` without /INT_ACK; confirm device-defined behavior. |
| D-13 | Loss-of-sync handling (drop to FIND_SOF on invalid non-token pair) | 2.6, A.4 | Corrupt one half; device RX returns to FIND_SOF. |
| D-14 | Inter-half timeout handling (abort ≥ **T_ifg** recommended) | 2.6 | Stall between halves; device aborts frame. |

### C.2 Backplane (Serial Profile)

| ID | Requirement (summary) | Spec § | How to verify |
| --- | --- | --- | --- |
| B-1 | Provides per-slot LVDS pairs; **100 Ω** term at receiver | 2.2 | SI inspection; measure differential impedance/termination. |
| B-2 | Supplies per-slot **3V3** within Core budget | 2.2 | Load test; current headroom for SERDES+logic. |
| B-3 | Decodes `(IOWin, IOMask, OpSel)`, asserts exactly one `/CS[slot]` | 2.5 | Address sweep; ensure single-slot selection. |
| B-4 | Forwards **full address** in frame (AL-selected width) | 2.5, 2.4.1 | Device echoes latched ADDR; verify wire image. |
| B-5 | **/READY** virtualization during framed ops | 2.5 | Observe CPU WAIT stretch equals frame duration. |
| B-6 | No frame for unmapped windows; CPU read → **all-ones** | 2.5 | Read unmapped; verify `0xFF/0xFFFF/0xFFFFFFFF`. |
| B-7 | **IDLE** preamble ≥ 16 tokens after reset/loss-of-lock | 2.6 | Scope link after reset; count tokens. |
| B-8 | Loss-of-sync recovery to **FIND_SOF** on bad pair | 2.6 | Inject parity-invalid non-token; observe state machine. |
| B-9 | **Contiguity**: no tokens inside frames; may terminate with **ERR** | 2.4.3 | Insert IDLE mid-frame → drop; inject ERR → abort. |
| B-10 | Timeout/abort policy: reads → **all-ones**, partial writes not committed | 2.4.3 | Stall device; verify CPU-side result and no writes applied. |
| B-11 | Interrupt routing per-slot; CPU **/CPU_ACK[k]** handling | 2.7, 2.9 | Assert device line; see CPU interrupt; observe ack flow. |
| B-12 | Mode-2: identify asserting slot; hold `/CS`; assert slot **/INT_ACK[k]**; issue vector read (`CONTROL=0x01`,`ADDR=0x00`,`FLAGS=0x00`) | 2.7 | Logic trace of ack cycle; capture request frame. |
| B-13 | Multiple claimants: return **0xFF** (don’t query any slot) | 2.7 | Two devices assert same channel; check CPU-side vector. |
| B-14 | NMI not vectored; don’t attempt vector read | 2.7 | NMI test; confirm no Serial vector transaction. |
| B-15 | Inter-half timeout **T_ifg ≥ 64 UI** (recommended) | 2.6 | Stall between halves; confirm abort behavior. |