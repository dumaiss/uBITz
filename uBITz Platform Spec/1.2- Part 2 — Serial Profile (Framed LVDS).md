...
| D-11 | Mode-2: while **/INT_ACK** asserted, READ at `ADDR=0x00` returns 8-bit vector (0xFF if not claimant) | 2.7          | Drive interrupt; ack; verify vector byte; test non-claim = 0xFF.     |
...
| B-11 | Interrupt routing per-slot; CPU **/CPU_ACK** handling                                                                              | 2.7, 2.9   | Assert device line; see CPU interrupt; observe ack flow.    |
| B-12 | Mode-2: identify asserting slot; hold `/CS`; assert slot **/INT_ACK**; issue vector read (`CONTROL=0x01`,`ADDR=0x00`,`FLAGS=0x00`) | 2.7        | Logic trace of ack cycle; capture request frame.            |
