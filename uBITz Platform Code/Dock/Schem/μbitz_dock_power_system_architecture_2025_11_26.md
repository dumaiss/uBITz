...
To ensure a clean start of the AON domain:

- A small **voltage supervisor IC** monitors 3V3_AON.
    
- The supervisor:
    
    - Holds the Dock MCU (and any AON logic) in reset until 3V3_AON is above threshold and stable for a defined delay (e.g., 100 ms).
        
    - Releases MCU reset only when AON is genuinely valid.
        

This avoids MCU brown-out glitches and makes the power-on behaviour deterministic.
...
Service USB Conn
 ...
 VBUS_PC_PROT ------------------------------+----------------------+
           ...
           VBUS_PD_SYS                                                      VBUS_PC_SYS
                 ...
                 |
                 |                              (AON Supervisor monitors 3V3_AON)
